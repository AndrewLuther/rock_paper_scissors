...
// Websocket server, noServer because we are using express server to simplify and make everything live in one place
...
const homepageClients = new Map<string, WebSocket>();
...
wss.on("connection", (ws, request) => {
  ...
  function didPlayerWin(
    ...
    otherSelection: "rock" | "paper" | "scissors"
  ) {
  ...
  function handleJoin(msg: ClientJoinMessage) {
    onHomepage = false;
    ...
    const helloPayload: HelloMessage = {
      clientId,
  ...
  function handleSelect(msg: ClientSelectMessage) {
    console.log(
      msg.clientId +
        " from game " +
        game.id +
  ...
  function handleHomepageJoin(msg: ClientHomepageJoinMessage) {
    onHomepage = true;
  ...
  ws.on(
    "message",
    (
      msg: ClientJoinMessage | ClientSelectMessage | ClientHomepageJoinMessage
    ) => {
  ...
  ws.on("close", () => {
    if (game) {
      ...
      if (game.clients.size == 0) {
        const gameDeleteMessage: HomepageGameDeleteMessage = {
          type: "gameDelete",
          gameId: game.id,
        };
...
function createGame(id?: string): Game {
  ...
  const newGameMessage: HomepageNewGameMessage = {
    type: "newGame",
    newGame: {
      id: game.id,
      numPlayers: game.clients.size,
